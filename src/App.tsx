          ...
          <main className="flex-1 lg:pl-64 overflow-y-auto p-4 sm:p-6 lg:p-8"><div className="max-w-6xl mx-auto">
            ...
            {!isLoading && <>
              {activeTab === 'dashboard' && <div className="space-y-8">
                ...
                {hasResearchResults && (<div onClick={() => setActiveTab('category-explorer')} className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs hover:border-amber-400 transition-colors cursor-pointer"><span className="text-[11px] font-semibold text-slate-500 uppercase block mb-1">Categories</span><div className="text-xl sm:text-2xl font-black text-slate-900 flex items-center justify-between"><span>{new Set(currentReport.niches.map(n => n.broadCategory?.trim() || 'Other')).size}</span><Compass className="w-5 h-5 text-purple-500" /></div><span className="text-[10px] text-slate-400 mt-1 block">Category Explorer</span></div>)}
              ...
              {(activeTab === 'category-explorer' || activeTab === 'competitors') && <CategoryExplorerView niches={currentReport.niches} topic={searchTopic} onExploreCategory={handleSearch} />}
