        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl border border-slate-200">
              <div>
                <div className="flex items-center gap-2">
                  <strong className="text-xs font-bold text-slate-900">Gemini 3.8 Flash Engine</strong>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                    settings.geminiConfigured
                      ? 'bg-emerald-100 text-emerald-800'
                      : 'bg-amber-100 text-amber-800'
                  }`}>
                    {settings.geminiConfigured ? 'Key Configured' : 'No Key'}
                  </span>
                </div>
                <span className="text-xs text-slate-500">Primary synthesis engine for qualitative market signals and gap detection</span>
              </div>
              <button
            ...
            <div className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl border border-slate-200">
              <div>
                <div className="flex items-center gap-2">
                  <strong className="text-xs font-bold text-slate-900">Groq LLaMA 3.3 70B Versatile</strong>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                    settings.groqConfigured
                      ? 'bg-emerald-100 text-emerald-800'
                      : 'bg-slate-200 text-slate-600'
                  }`}>
                    {settings.groqConfigured ? 'Key Configured' : 'Standby / Optional'}
                  </span>
                </div>
                <span className="text-xs text-slate-500">High-speed fallback & rollover engine when Gemini is rate-limited or unavailable</span>
              </div>
              <span className={`px-3 py-1.5 rounded-lg text-xs font-bold ${
                ...
                {settings.groqConfigured ? 'Rollup Active' : 'Idle'}
              </span>
            </div>

            {/* Rollup Status Callout */}
            <div className="p-3.5 bg-blue-50/70 border border-blue-200 rounded-xl flex items-start gap-2.5">
              <div className="w-2 h-2 rounded-full bg-blue-500 mt-1.5 shrink-0" />
              <div className="text-xs text-blue-900 space-y-1">
                <span className="font-bold block">Intelligent Multi-Provider Rollup Active</span>
                <p className="text-blue-800">
                  The API automatically checks both <code className="bg-blue-100 px-1 py-0.5 rounded text-[11px]">GEMINI_API_KEY</code> and <code className="bg-blue-100 px-1 py-0.5 rounded text-[11px]">GROQ_API_KEY</code>. If both are available, Gemini is prioritized with instant zero-downtime rollover to Groq LLaMA 3.3 70B if rate limits or errors occur. If neither key is provided, the resilient heuristic model prevents downtime.
                </p>
              </div>
            </div>
