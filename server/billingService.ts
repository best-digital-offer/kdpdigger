...
export class BillingService {
   ...
   */
  applyPlanToUser(userId: string, planId: string): { success: boolean; newCredits: number; planName: string } {
    const plans = db.getPlans();
    const plan = plans.find(p => p.id === planId);
    if (!plan) {
    ...
    const updatedUser = db.addUserCredits(userId, plan.credits, plan.name, plan.durationDays);
    if (!updatedUser) {
